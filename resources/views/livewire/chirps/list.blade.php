            ...
            <div class="flex-1">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <div>
                        ...
                        @else
                            <a href="{{route('user', $chirp->user)}}" class="text-gray-800">{{ $chirp->user->name }}</a>
                        @endif
                    ...
                    @if ($chirp->user->is(auth()->user()))
                        <x-dropdown>
                            ...
                            <x-slot name="content">
                                <x-dropdown-link wire:click="edit({{ $chirp->id }})" class="cursor-pointer">
                                    {{ __('Edit') }}
                                </x-dropdown-link>
                                <x-dropdown-link wire:click="delete({{ $chirp->id }})" class="cursor-pointer" wire:confirm="Are you sure to delete this chirp?">
                                    {{ __('Delete') }}
                                </x-dropdown-link>
                            </x-slot>
                        </x-dropdown>
